...
// printf("i:%d j:%d | bcDim.x:%d  bkDim.y:%d | blockId.x:%d bkId.y:%d\n", i, j,blockDim.x,blockDim.y,blockIdx.x,blockIdx.y );
	...
	for(int ii=0; ii< *C_dev ; ii++){
      		...
      		printf("dc[%d](%d)+= da[%d](%d) * db[%d](%d)\n",i*(*Cb_dev)+j, dc[i*(*Cb_dev)+j], i*(*C_dev)+ii,da[i*(*C_dev)+ii], ii*(*Cb_dev) + j,db[ii*(*Cb_dev) + j]);
...
//	dimBlock.y=2;
  ...
  printf("dT.x:%d dT.y:%d | dB.x:%d dB.y:%d",dimThreads.x, dimThreads.y, dimBlock.x, dimBlock.y);
