...
int attackqueens(int oi, int oj, char a, char** board, int m, int n)
  ...
  void queens (int sti, int stj ,char ** board, int m, int n, int k) {
    ...
    int rainhas_atual=0, pcl = busca_primeira_linha_sem_peca(board,m,n, 'K');;

    while(sti < n && stj < n && rainhas_atual < m){

      //controle
      int ii=0, jj = pcl+ (omp_get_num_threads()+1)* (((m-pcl)*n)/omp_get_max_threads());
      // int ii=0, jj = pcl+ i* ((m*n-k)/omp_get_max_threads());//comecando pela primeira linha sem cavaleiro
      printf("jj: %d ;; nova dimensao %d x %d\n", jj, m-pcl, n );
...
//       printf("sti:%d; stj:%d\n",sti,stj );
      ...
      if(k<=0){
        ...
        //se eu obtivesse a primeira coluna livre (pcl) na qual nao tem cavaleiro e comecasse por ela, seria mais rapido, pois perdemos muito tempo comecando por posicoes invalidas
        int pcl = busca_primeira_linha_sem_peca(new_board,m,n, 'K');
        printf("primeira linha sem cavaleiro:%d\n",pcl );

        // if(k<m){//se k>=m uma linha foi completamente preenchida = nao tem como posicionar rainhas
        if(pcl!=-1){//tem pelo menos uma linha sem cavaleiro
        //escalonar a tarefa
        #pragma omp parallel for
        for(int i=0; i<omp_get_max_threads(); i++){

          // int ii=0, jj = i* ((m*n-k)/omp_get_max_threads());
          //m*n = quantidade de celulas da matriz
          //(m-pcl)* - k*n = cada linha que possui um cavaleiro, se torna inutil, portanto, devemos
          int ii=0, jj = pcl+ i* (((m-pcl)*n)/omp_get_max_threads());
          // int ii=0, jj = pcl+ i* ((m*n-k)/omp_get_max_threads());//comecando pela primeira linha sem cavaleiro
          printf("jj: %d ;; nova dimensao %d x %d\n", jj, m-pcl, n );
        ...
        printf("Nao eh possivel posicionar uma rainha K>=M (matriz nao possui linha livre)\n");
      }
